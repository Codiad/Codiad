...
switch($_GET['action']){
    ...
    $path = $_GET['path'];
    ?>
    <label><?php lang("Close Unsaved File?"); ?></label>
    
    <pre><?php echo($path); ?></pre>

    <button class="btn-left" onclick="save_and_close('<?php echo($path); ?>'); return false;"><?php lang("Save&Close"); ?></button>
    <button class="btn-mid" onclick="close_without_save('<?php echo($path); ?>'); return false;"><?php lang("Discard Changes"); ?></button>
